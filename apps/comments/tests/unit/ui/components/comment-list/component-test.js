...
module('Unit | Component | comment list', function(hooks) {
  ...
  module('getter/setter: filteredNewComments', function() {
    test('should return comments from @newComments that aren\'t already in @comments', async function(assert) {
      assert.expect(1);

      // Arrange
      const factory = this.owner.factoryFor('component:comment-list');
      const component = await factory.create({
        '--comments': ['foo', 'bar', 'dee'],
        '--newComments': ['foo', 'woo'],
      });

      // Act
      const result = component.get('filteredNewComments');

      // Assert
      assert.deepEqual(result, ['woo']);
    });
  });
