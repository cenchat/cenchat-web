...
module('Unit | Route | chats/chat/messages', function (hooks) {
  setupTest(hooks);
  ...
  test('should return chat messages as model', async function (assert) {
    ...
    // Arrange
    const route = this.owner.lookup('route:chats/chat/messages');

    sinon.stub(route, 'modelFor').withArgs('chats.chat').returns({
